...
import { networkInfo, vaultInfo, calculateInvestmentGrowth } from "@/lib/helpers"
...
export default function InvestmentForm({ onCalculate }: InvestmentFormProps) {
  ...
  const handleChange = (name: keyof FormData, value: unknown) => {
    ...
    if (name === 'networkId' && apyData) {
      ...
      setFormData(prev => ({ ...prev!, networkId: value as string, vaultId: firstVaultId }));
    }
  };
  ...
  const isSelectedVaultUSD = formData.vaultId ? 
    vaultInfo[formData.vaultId]?.isUSD ?? true : true;
          ...
          <div className="grid gap-6" suppressHydrationWarning>
            <div className="grid gap-3">
              ...
              >
                ...
                <SelectContent>
                  {apyData && Object.keys(apyData.data).map((networkId) => (
                    <SelectItem key={networkId} value={networkId} className="flex items-center gap-2">
            ...
            <div className="grid gap-3">
              ...
              >
                ...
                <SelectContent>
                  {formData.networkId && apyData && Object.keys(apyData.data[formData.networkId]).map((vaultId) => (
                    <SelectItem key={vaultId} value={vaultId}>
                      <div className="flex items-center gap-2">
                        ...
                        {(Number(apyData.data[formData.networkId][vaultId]) * 100).toFixed(2)}%
                      </div>
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            {!isSelectedVaultUSD && (
              <div className="flex items-center space-x-2">
                ...
                />
                <Label htmlFor="useUSD">Input in USD instead of native token</Label>
              </div>
            )}
            ...
            <div className="grid gap-3">
              ...
              <div className="flex items-center">
                <Input 
                  ...
                  onChange={(e) => handleChange('initialAmount', Number(e.target.value))}
                  className="w-full"
                />
                <span className="ml-2">
                  {formData.useUSD || isSelectedVaultUSD ? 'USD' : vaultInfo[formData.vaultId]?.assetType || ''}
                </span>
              </div>
            </div>
            ...
            <div className="grid gap-3">
              ...
              <div className="flex gap-2">
                <Input 
                  ...
                  onChange={(e) => handleChange('timeFrameValue', Number(e.target.value))}
                  className="flex-1"
                />
                <Select
                  value={formData.timeFrameUnit}
                  onValueChange={(value: 'months' | 'years') => handleChange('timeFrameUnit', value)}
                >
